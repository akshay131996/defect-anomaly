#!/usr/bin/env python3
"""
run_ref_official.py: Official Amazon PatchCore Ensemble on MVTec AD 2 test_public.
Supports --mode 256, 512, halfnative.
...
- Smoothing: Gaussian filter sigma=4 on anomaly map (standard PatchCore RescaleSegmentor)
...
def run_scenario(sc, mode, data_root):
    t0 = time.time()
    r = os.path.join(data_root, sc)
    ...
    if mode == "256":
        ...
        w_in, h_in = 512, 512
        batch_size = 8
    elif mode == "halfnative":
        w_in, h_in = (w_nat // 2), (h_nat // 2)
        # Ensure dimensions are at least multiples of 8 for feature downsampling
        w_in = max(32, (w_in // 8) * 8)
        h_in = max(32, (h_in // 8) * 8)
        batch_size = 4
    else:
    ...
    train_ds = PathDataset(train_paths, transform=tfm)
    train_loader = torch.utils.data.DataLoader(train_ds, batch_size=batch_size, shuffle=False, num_workers=4, pin_memory=True)

    test_paths = good_paths + bad_paths
    test_ds = PathDataset(test_paths, transform=tfm)
    ...
    for name, layers in MODELS_CONFIG:
        print(f"  [{sc}] fitting {name} ({w_in}x{h_in})...", flush=True)
    ...
    for segs in ensemble_segs:
        segs = np.array(segs, dtype=np.float32)  # (N_test, h_in, w_in)
    ...
    final_maps = np.mean(norm_segs, axis=0)  # (N_test, h_in, w_in)
    ...
    # Standard evaluation shape:
    # If square mode (256, 512): eval on 512x512
    # If halfnative: eval on aspect-preserving (eval_w, eval_h)
    if mode in ("256", "512"):
        eval_w, eval_h = 512, 512
    else:
        eval_w, eval_h = aspect_dimensions(w_nat, h_nat, target_img=512, stride=32)

    m_good_eval = [np.array(Image.fromarray(m).resize((eval_w, eval_h), Image.BILINEAR)) for m in m_good]
    m_bad_eval = [np.array(Image.fromarray(m).resize((eval_w, eval_h), Image.BILINEAR)) for m in m_bad]
    ...
    for p in masks_paths:
        ...
        # Resize clean_labels to (eval_w, eval_h) with NEAREST
    ...
    lo = min(float(m.min()) for m in (m_good_eval + m_bad_eval))
    hi = max(float(m.max()) for m in (m_good_eval + m_bad_eval))

    res = evaluate(m_good_eval, m_bad_eval, masks, lo, hi, valid=None, region_labels=region_labels)
...
def main():
    parser = argparse.ArgumentParser(description="Official Amazon PatchCore Ensemble Evaluation")
    parser.add_argument("--mode", type=str, choices=["256", "512", "halfnative"], default="512",
                        help="Resolution mode: 256 (squash), 512 (squash), or halfnative (50% per dim)")
    parser.add_argument("--run-id", type=str, default=None)
    parser.add_argument("--out", type=str, default=None)
    parser.add_argument("--data-root", type=str, default="/opt/ad2/mvtec_ad_2")
    parser.add_argument("--scenarios", nargs="+", default=SCENARIOS)
    args = parser.parse_args()

    run_id = args.run_id or f"REF-official-{args.mode}"
    out_path = args.out or f"/workspace/outputs/runs/{run_id}.json"

    print(f"Starting {run_id} (Amazon PatchCore Ensemble @ {args.mode})...", flush=True)
    t_start = time.time()
    results = {
        "run_id": run_id,
        "hypothesis": f"Official Amazon PatchCore Ensemble reproduced on MVTec AD 2 test_public at {args.mode} resolution with native region evaluator",
        "command": f"python run_ref_official.py --mode {args.mode} --run-id {run_id} --out {out_path}",
        ...
        "config": {
            ...
            "geometry": "squash" if args.mode in ("256", "512") else "halfnative",
    ...
    for sc in args.scenarios:
        results["scenarios"][sc] = run_scenario(sc, args.mode, args.data_root)
    ...
    print(f"\nCompleted {run_id} in {total_wall}s.")
